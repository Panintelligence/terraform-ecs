...
resource "aws_db_instance" "database" {
  ...
  db_subnet_group_name = aws_db_subnet_group.pi.name
  engine = "mariadb"
  engine_version = var.engine_version
  username = var.dashboard_db_username
  password = var.dashboard_db_password
  parameter_group_name = aws_db_parameter_group.pi.name
...
resource "aws_db_subnet_group" "pi" {
...
resource "aws_db_parameter_group" "pi" {
